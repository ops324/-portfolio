...
| 読み仮名 | 「たきもと てつや」（Shippori Mincho 0.72rem、letter-spacing 0.55em、`--muted`、aria-hidden） |
| 名前 | 「滝本 哲也」（**Shippori Mincho B1**、`clamp(3.6rem, 9vw, 7.2rem)`、weight 400、`--ink`） |
| 説明文 | Noto Sans JP、`--fs-body`、`var(--text)`、line-height 2.15 |
| 縦書き題字 | 右端に「作品集 — 二〇二六」（`writing-mode: vertical-rl`、明朝 0.75rem、`--muted-2`、860px未満非表示） |
...
| 入場演出 | `document.fonts.ready`（上限2.5s）待ち → `#page-intro` を `scaleY` 巻き上げ → sub→kana→**名前を文字単位マスク出現（SplitText `type:'chars'` `mask:'chars'` stagger 0.06）**→desc→縦書き/指標→ライン（GSAP timeline）。SplitText欠落時は名前を行送りにフォールバック |
...
| リビール | eyebrow→meta→divider→補助 の順に GSAP で順次表示 |
| 「軸」本文演出 | **薄墨→墨の文字色スクロールスクラブ**（SplitText chars、`#cfc9bd`→`#1a1a1c`、scrub 0.8）。全行が1行に収まる幅のみ適用（CJK禁則保護）、狭幅・SplitText欠落・reduced-motion時は行送り表示 |
...
| 奥付行 | 「設計・制作 — 滝本 哲也」「書体 — しっぽり明朝 B1 / Noto Sans JP / Inter」「© 2026 Tetsuya Takimoto」（`--fs-micro`・`--muted-2`） |
...
| 860px 以下 | works を1カラム縦積み（サムネ4:3復帰・ゴースト索引縮小）、`--rhythm` 縮小、ナビ gap 22px・padding 20px。hero の縦書き題字・スクロール指標非表示、WebGL靄停止（CSS滲みへ） |
...
| About | サブ要素を段階的に stagger 表示。「軸」は薄墨→墨の文字スクラブ（3.3参照） |
